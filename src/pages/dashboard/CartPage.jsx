...
const CartPage = ({ iscart }) => {
  ...
  const handleIncreaseQuantity = async (itemId) => {
    if (cartItems[itemId].qty < 10) {
      ...
      await fetchreq(
        "GET",
        `updateCartqty/${temp[itemId].cartid}/${temp[itemId].qty}`,
        {}
      );
    }
  };
  ...
  return (
    <div className="overflow-y-scroll h-[100dvh] cart-page bg-[#E2E8F0] p-8 rounded-md">
      <div className="flex flex-wrap gap-2">
        <Link className="atc" to={"/dashboard/shop"}>
          Go to Shop
        </Link>
        <Link className="atc" to={iscart ? "/orders" : "/cart"}>
          Go to {iscart ? "Orders" : "Cart"}
        </Link>
      </div>
      ...
      {cartItems &&
        cartItems.map((item, index) => {
          return (
            <div
              key={item.cartid}
              className="cart-item bg-primary-light rounded-md p-4 justify-between mb-4 flex items-center flex-wrap"
            >
              <img
              ...
              <div className="item-details ml-4">
                ...
                <p className="text-gray-600 mt-2 flex flex-col">
                  Price: ₹{item.Price}{" "}
                  <div>
                    {item.qty > 1 && `total: ₹${item.Price * item.qty}`}
                  </div>
                </p>
              </div>
              <div className="w-[60%] ">
